(function () {
    // MQTT configuración por defecto (puede ser editable por UI, pero estos son los valores por defecto)
    ...
    const MQTT_ENABLE_LEGACY_STEP_ID_REMAP = false;
    ...
    function resolveLegacyMqttStepIdRemap(stepId, cycleNumber, stepRole) {
        const rawStepId = Number(stepId);
        if (!MQTT_ENABLE_LEGACY_STEP_ID_REMAP) {
            return {
                storedStepId: rawStepId,
                plannedStepId: rawStepId,
                legacyRemapApplied: false,
                legacyRemapReason: 'disabled'
            };
        }

        let plannedStepId = rawStepId;
        const normalizedCycleNumber = Number.isFinite(Number(cycleNumber)) ? Number(cycleNumber) : null;
        const normalizedRole = String(stepRole || '').toLowerCase();

        // LEGACY ONLY: historical pallet pick/place remap kept for backwards compatibility.
        // Do not enable by default for new executions; the robot already emits the correct step_id.
        const palletTargetPickFirst = 2;
        const palletTargetPickStride = 8;
        const palletTargetPlaceFirst = 6;
        const palletTargetPlaceStride = 8;
        if (normalizedCycleNumber && (normalizedRole === 'pick' || normalizedRole === 'place')) {
            if (normalizedRole === 'pick') {
                plannedStepId = palletTargetPickFirst + (normalizedCycleNumber - 1) * palletTargetPickStride;
            } else if (normalizedRole === 'place') {
                plannedStepId = palletTargetPlaceFirst + (normalizedCycleNumber - 1) * palletTargetPlaceStride;
            }
        }
        ...
        return {
            storedStepId: rawStepId,
            plannedStepId,
            legacyRemapApplied: plannedStepId !== rawStepId,
            legacyRemapReason: plannedStepId !== rawStepId ? 'pallet_cycle_formula' : 'identity'
        };
    }
    ...
    function getMqttBrokerUrl() {
        try {
            const stored = String(localStorage.getItem(MQTT_BROKER_STORAGE_KEY) || '').trim();
            return stored || MQTT_WS_URL;
        } catch {
            return MQTT_WS_URL;
        }
    }
    ...
    function getMqttTopic() {
        try {
            const stored = String(localStorage.getItem(MQTT_TOPIC_STORAGE_KEY) || '').trim();
            return stored || MQTT_TOPIC;
        } catch {
            return MQTT_TOPIC;
        }
    }
    ...
    function notifyMqttUiStatus(info = {}) {
        const payload = { brokerUrl: getMqttBrokerUrl(), topic: getMqttTopic(), ...info };
        const status = String(payload?.status || 'disconnected').toLowerCase();
        console.log(`[MQTT UI] status=${status}`);

        if (window.SimulatorAPI?.setMqttConnectionStatus) {
            window.SimulatorAPI.setMqttConnectionStatus(payload);
            return;
        }

        const el = document.getElementById('mqttStatusIndicator');
        if (!el) return;

        let text = 'MQTT: 🔴 desconectado';
        if (status === 'connecting') text = 'MQTT: 🟠 conectando...';
        else if (status === 'connected' || status === 'subscribed') text = 'MQTT: 🟢 conectado';
        else if (status === 'reconnecting') text = 'MQTT: 🟡 reconectando...';
        else if (status === 'offline') text = 'MQTT: 🔴 sin conexión';
        else if (status === 'error') text = 'MQTT: 🔴 error';

        el.textContent = text;
        el.title = [
            payload?.brokerUrl ? `Broker: ${payload.brokerUrl}` : '',
    ...
    function adaptMqttStepCapture(raw) {
        ...
        // Compatibilidad: aceptar payloads antiguos
        const tcp = raw.tcp_position_mm || raw.tcp_position;
        ...
        // Permitir step_id como string o número
        const stepId = Number(raw.step_id);
        if (!Number.isFinite(stepId)) return null;

        // event_counter o event_seq
        const eventCounter = Number.isFinite(Number(raw.event_counter)) ? Number(raw.event_counter) : (Number.isFinite(Number(raw.event_seq)) ? Number(raw.event_seq) : null);

        // timestamp
        const ts = Date.parse(raw.timestamp);
        if (!Number.isFinite(ts)) return null;

        // snapshot_short y snapshot_id
        const mqttSnapshotId = String(raw.snapshot_id || '').trim() || null;
        const parsedSnapshotShort = Number(raw.snapshot_short);
        const mqttSnapshotShort = Number.isFinite(parsedSnapshotShort) ? (parsedSnapshotShort & 0xFFFF) : null;
        ...
        // Orientación: usar tcp_orientation si viene, si no tcp_orientation_rad, si no rx/ry/rz
        let rx = null, ry = null, rz = null;
        if (raw.tcp_orientation && typeof raw.tcp_orientation === 'object') {
            rx = Number(raw.tcp_orientation.rx);
            ry = Number(raw.tcp_orientation.ry);
            rz = Number(raw.tcp_orientation.rz);
        } else if (raw.tcp_orientation_rad && typeof raw.tcp_orientation_rad === 'object') {
            rx = Number(raw.tcp_orientation_rad.rx);
            ry = Number(raw.tcp_orientation_rad.ry);
            rz = Number(raw.tcp_orientation_rad.rz);
        } else {
            if (Number.isFinite(raw.rx)) rx = Number(raw.rx);
            if (Number.isFinite(raw.ry)) ry = Number(raw.ry);
            if (Number.isFinite(raw.rz)) rz = Number(raw.rz);
        }
        if (!Number.isFinite(rx)) rx = null;
        if (!Number.isFinite(ry)) ry = null;
        if (!Number.isFinite(rz)) rz = null;

        // message_type y schema_version
        const messageType = String(raw.message_type || '').trim();
        const schemaVersion = String(raw.schema_version || '').trim();
        const isStepCapture = messageType === 'step_capture' && schemaVersion.startsWith('step_capture_socket_clean_');

        // Ciclo y roles
        const eventTypeRaw = String(raw.event_type || '').toLowerCase();
        const markerType = String(raw.marker_type || '').toLowerCase();
        const stepType = String(raw.step_type || '').toLowerCase();
        const stepRole = String(raw.step_role || '').toLowerCase();
        const cycleNumber = Number.isFinite(Number(raw.cycle_number)) ? Number(raw.cycle_number) : null;
        const cycleLabel = raw.cycle_label || null;
        const cycleId = raw.cycle_id || null;
        const cycleAssigned = raw.cycle_assigned || null;
        const cycleSource = raw.cycle_source || null;
        const stepRoleLabel = raw.step_role_label || null;
        const stepIndexInCycle = Number.isFinite(Number(raw.step_index_in_cycle)) ? Number(raw.step_index_in_cycle) : null;
        const stepSource = raw.step_source || null;
        const tsNodeRedPublish = raw.ts_nodered_publish || null;

        // Detección de eventos de ciclo
        let isCycleStart = false, isCycleEnd = false, isCycleMarker = false;
        if (eventTypeRaw === 'cycle_start' || markerType === 'cycle_start' || stepId === 10000) isCycleStart = true;
        if (eventTypeRaw === 'cycle_end' || markerType === 'cycle_end' || stepId === 20000) isCycleEnd = true;
        isCycleMarker = isCycleStart || isCycleEnd;

        let eventType = 'step';
        if (isCycleStart) eventType = 'cycle_start';
        else if (isCycleEnd) eventType = 'cycle_end';
        else if (eventTypeRaw) eventType = eventTypeRaw;

        const stepIdResolution = resolveLegacyMqttStepIdRemap(stepId, cycleNumber, stepRole);
        const storedStepId = stepIdResolution.storedStepId;
        const plannedStepId = stepIdResolution.plannedStepId;

        // event_id único
        const eventId = `${raw.snapshot_short || ''}_${eventCounter || ''}_${cycleNumber || ''}_${stepId || ''}_${ts || ''}`;

        // LOGS
        if (isCycleMarker) {
            const label = isCycleStart ? 'start' : 'end';
            console.log(`[MQTT STEP_CAPTURE] topic=${MQTT_TOPIC} schema=${schemaVersion} type=${eventType} step=${stepId} cycle=${cycleNumber}`);
            console.log(`[TELEMETRY CYCLE] ${label} snapshot=${mqttSnapshotId} cycle=${cycleNumber} event_counter=${eventCounter}`);
        } else if (eventType === 'work_step' || stepType === 'work_step') {
            console.log(`[TELEMETRY STEP] role=${stepRole} raw_step=${stepId} planned_step=${plannedStepId} cycle=${cycleNumber}`);
        }
        ...
        return {
            ...
            step_role: stepRole,
            step_role_label: stepRoleLabel,
            step_type: stepType,
            step_index_in_cycle: stepIndexInCycle,
            step_source: stepSource,
            ts_nodered_publish: tsNodeRedPublish,
            meta: {
                event_counter: eventCounter,
                source: sourceType,
                raw_step_id: stepId,
                stored_step_id: storedStepId,
                legacy_step_id_remap_applied: stepIdResolution.legacyRemapApplied,
                legacy_step_id_remap_reason: stepIdResolution.legacyRemapReason,
    ...
    function isComparableTelemetryStep(adapted) {
            const stepId = Number(adapted?.step_id);
            const rawStepId = Number(adapted?.raw_step_id ?? adapted?.step_id);

      if (!Number.isFinite(stepId)) return false;

      const eventType = String(adapted?.event_type || '').toLowerCase();
      const stepType = String(adapted?.step_type || '').toLowerCase();

      if (adapted?.isCycleMarker) return false;
      if (rawStepId === 10000 || rawStepId === 20000) return false;
      if (stepId === 10000 || stepId === 20000) return false;

      return (
        eventType === 'step' ||
        eventType === 'work_step' ||
        stepType === 'work_step' ||
        (stepId > 0 && stepId < 9000)
      );
    }
    ...
    async function pushMqttResultToSimulator(adapted) {
        ...
        // Persistir step en IDB solo si es punto real comparable
        if (isComparableTelemetryStep(adapted)) {
            const storedStepId = Number(adapted.step_id);
            const rawStepId = Number(adapted.raw_step_id ?? adapted.step_id);

            console.log('[MQTT STEP ID]', {
                raw_step_id: rawStepId,
                stored_step_id: storedStepId,
            ...
            if (typeof window.upsertRunStepResult === 'function') {
                await window.upsertRunStepResult(runId, {
                    step_id: storedStepId,
                    raw_step_id: rawStepId,
                    pose: adapted.pose,
                    ...
                    meta: {
                        ...
                        step_type: adapted.step_type,
                        step_role: adapted.step_role,
                        cycle_number: adapted.cycle_number,
                        raw_step_id: rawStepId,
                        stored_step_id: storedStepId,
                        planned_step_id: adapted.planned_step_id
                    }
                }).catch((error) => {
                    console.warn('[telemetry] persist mqtt run error:', error);
                });
                console.log(`[TELEMETRY RESULT] saved run=${runId} step=${storedStepId} raw_step=${rawStepId} role=${adapted.step_role || '?'} cycle=${adapted.cycle_number || '?'}`);
            }
            ...
            if (Array.isArray(planned)) {
                ...
                if (!validIds.has(storedStepId)) {
                    // Solo advertir si no es marcador de ciclo
                    if (storedStepId < 10000 || storedStepId > 20000) {
                        console.warn('[telemetry] step_id fuera del plan actual:', storedStepId);
                    }
                    return;
                }
            }

            const idx = actual.findIndex((item) => item && item.step_id === storedStepId);
            if (idx >= 0) {
                actual[idx] = { ...adapted, step_id: storedStepId, raw_step_id: rawStepId };
            } else {
                actual.push({ ...adapted, step_id: storedStepId, raw_step_id: rawStepId });
            }
        ...
        } else if (adapted.event_type === 'cycle_start' || adapted.event_type === 'cycle_end') {
            // Loguear evento de ciclo, no comparar ni insertar en resultsByStep
            const label = adapted.event_type === 'cycle_start' ? 'start' : 'end';
            console.log(`[TELEMETRY CYCLE] ${label} snapshot=${mqttSnapshotId} event_counter=${eventCounter}`);
            // Aquí se podría guardar en una colección separada de eventos de ciclo si se desea
        } else {
            console.log(`[TELEMETRY RESULT] ignored non-comparable event_type=${adapted.event_type} step=${adapted.step_id || '?'}`);
        }
    }
    ...
    function connectMqttTelemetry() {
        ...
        mqttClient.on('message', (topic, payload) => {
            try {
                ...
                const messageType = String(raw.message_type || '').trim();
                const schemaVersion = String(raw.schema_version || '').trim();
                if (messageType !== 'step_capture') {
                    // Ignorar mensajes que no sean step_capture
                    return;
                }
                if (!(schemaVersion === 'step_capture_socket_clean_v8_counted_cycles' || schemaVersion.startsWith('step_capture_socket_clean_'))) {
                    // Ignorar schemas no válidos
                    return;
                }
                // Log de recepción
                const eventType = String(raw.event_type || '').trim();
                const stepId = raw.step_id;
                const cycleNumber = raw.cycle_number;
                const stepRole = raw.step_role;
                console.log(`[MQTT STEP_CAPTURE] topic=${topic} schema=${schemaVersion} event_type=${eventType} step=${stepId} cycle=${cycleNumber} role=${stepRole}`);
        ...
        mqttClient.on('offline', () => notifyMqttUiStatus({ status: 'offline', clientId, brokerUrl, topic }));
        mqttClient.on('close', () => notifyMqttUiStatus({ status: 'disconnected', clientId, brokerUrl, topic }));
        mqttClient.on('reconnect', () => notifyMqttUiStatus({ status: 'reconnecting', clientId, brokerUrl, topic }));
